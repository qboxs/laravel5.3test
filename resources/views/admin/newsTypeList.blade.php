    ...
    <div class="block">
        <form style="margin-top: 2rem;" class="form-horizontal" action="">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <input type="text" class="form-control" name="title" id="title" value="" placeholder="类别标题" required="required">
                </div>
                <label for="parent_id" class="col-sm-1 control-label">上级分类</label>
                <div class="col-sm-3">
                    <input type="text" class="form-control" name="parent_id" id="parent_id" value="0" placeholder="上级分类，没有默认填写1" required="required">
                </div>
                <label for="levels" class="col-sm-1 control-label">层级</label>
                <div class="col-sm-3">
                    <input type="text" class="form-control" name="levels" id="levels" value="1" placeholder="层级，默认填写1" required="required">
                </div>
            </div>
            <div class="form-group">
                <label for="detail" class="col-sm-1 control-label">描述</label>
                <div class="col-sm-11">
                    <input type="text" class="form-control" name="detail" id="detail" value="" placeholder="描述" required="required">
                </div>
            </div>
    ...
    <script>
        $(function() {
            ...
            $(document).on('click','.glyphicon-trash',function () {
                ...
                if(confirm('你确定要删除吗?')){
                    $.post("{{url('admin/newsType')}}/"+id, { "_method": "DELETE","_token": "{{ csrf_token() }}"},
                        function(data){
            ...
            $(document).on('click','.submit-btn',function () {
                ...
                if(confirm('你确定要添加吗?')){
                    $.post("{{url('admin/newsType')}}", {"_token": "{{ csrf_token() }}","title":title,"parent_id":parent_id,"levels":levels,"detail":detail,"status":1},
                        function(data){
                            //window.location.reload(true);
                            jqwidgetsLoad();
                        }, "json");
                ...
                $("#treeGrid").jqxTreeGrid(
                    {
                        ...
                        columns: [
                            { text: 'ID', editable: false, dataField: 'pk_id', width: 100 },
                    ...
                    $.post("{{url('admin/newsType')}}/"+pk_id, {"_method":"PUT","_token": "{{ csrf_token() }}","title":title,"parent_id":parent_id,"levels":levels,"detail":detail,"status":1},
                        function(data){
                            jqwidgetsLoad();
                        }, "json");
                });
