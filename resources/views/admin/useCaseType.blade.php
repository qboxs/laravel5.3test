...
@section('content')
    <ol class="breadcrumb">
        ...
        <li><a href="{{url('admin/useCaseType')}}">产品分类</a></li>
        <li class="active">@if (!empty($obj->pk_id))编辑 @else 添加 @endif</li>
    </ol>
    <div class="block">
        <form class="form-horizontal" action="@if (empty($obj->pk_id)){{url('admin/useCaseType')}} @else {{url('admin/useCaseType/'.$obj->pk_id)}} @endif" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
