...
@section('content')
    <ol class="breadcrumb">
        ...
        <li><a href="{{url('admin/useCaseType')}}">应用案例</a></li>
    ...
    <div class="block">
        <form style="margin-top: 2rem;" class="form-horizontal" action="">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-3">
                    <input type="text" class="form-control" name="parent_id" id="parent_id" value="1" placeholder="上级分类，没有默认填写1" required="required">
                </div>
    ...
    <script>
        $(function() {
            $(document).on('click','.glyphicon-trash',function () {
                ...
                if(confirm('你确定要删除吗?')){
                    $.post("{{url('admin/useCaseType')}}/"+id, { "_method": "DELETE","_token": "{{ csrf_token() }}"},
                        function(data){
            ...
            $(document).on('click','.submit-btn',function () {
                ...
                if(confirm('你确定要添加吗?')){
                    $.post("{{url('admin/useCaseType')}}", {"_token": "{{ csrf_token() }}","title":title,"parent_id":parent_id,"levels":levels,"detail":detail,"status":1},
                        function(data){
                            window.location.reload(true);
                        }, "json");
                }
            ...
            $("#treeGrid").jqxTreeGrid(
                {
                    ...
                    $.post("{{url('admin/useCaseType')}}/"+pk_id, {"_method":"PUT","_token": "{{ csrf_token() }}","title":title,"parent_id":parent_id,"levels":levels,"detail":detail,"status":1},
                        function(data){
                            window.location.reload(true);
                        }, "json");
                });
