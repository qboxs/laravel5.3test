...
@section('content')
    <ol class="breadcrumb">
        ...
        <li><a href="{{url('admin/useCaseInfo')}}">产品信息</a></li>
        <li class="active">@if (!empty($obj->pk_id))编辑 @else 添加 @endif</li>
    </ol>
    <div class="block">
        <form class="form-horizontal" style="margin-top: 3rem" action="@if (empty($obj->pk_id)){{url('admin/useCaseInfo')}} @else {{url('admin/useCaseInfo/'.$obj->pk_id)}} @endif" method="post" enctype="multipart/form-data" onsubmit="return saves();">
            {{ csrf_field() }}
